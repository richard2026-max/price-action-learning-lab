...
# 测试：cd apps/api && .venv/Scripts/python -m pytest tests/   （92 通过，2026-08-28 全量实测复核）
...
| MVP-C 结构层 | ✅ 第一批：swing（右侧N=3确认）/pullback_leg（净漂移上下文）/hl_counting（H1-H4·L1-L4 状态机，second_entry 标注）；profile=mvp-c-0.1.0；前端已构建；**92 测试全绿（2026-08-28 实测复核：92 passed）** |
...
3. 后置：~~sealed exam set~~ **已建成**（Batch 15：Blake2b 确定性哈希分层划分约 15%，普通回放/扫描自动排除封存日，封存日 403 隔离，仅 mode='exam' 可访问；纯函数式划分，新数据入库自动生效，无需迁移）。知识库与 AI provider 亦已落地（AI 决策教练 + 知识检索，DeepSeek provider，见 Batch 26+）。
