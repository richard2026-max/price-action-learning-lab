...
- Batch 24 完成（支持历史走势预热天数 + 图表与弹窗放大 + 盘前数据说明，2026-08-23）：
...
- Batch 27 完成（AI 对照复盘多层容错解析与排版优化，2026-08-26）：
  1. 提示词约束加固：强制中文输出、纯文本字段限制、严禁外层反引号代码块包裹，明确结构化 references 定义。
  2. 后端多重容错：支持纯 JSON、```json 代码块、正则字段提取兜底、自然语言标志分段（【原书依据】等）及引用深度解析。
  3. 前端双重防护：增加 cleanLayerText 与 renderLayerBody 段落排版，杜绝任何原始 JSON/反引号泄漏到 UI，增强引用摘录展示。
  4. 验证：后端全量 89 项测试全绿；Ruff 0 违规；Mypy 0 错误；前端 npm run build 打包成功；实机端到端验证通过。

- Batch 28 完成（课件原版形态图示与过往历史SPY真实走势图直出，2026-08-26）：
  1. 术语层级更新：三层诊断正式定名为【01 阿布价格行为学课件原意】、【02 当前行情客观事实】、【03 教练解释】。
...
- Batch 30 完成（图表训练体验升级：OHLC图例/北京时间轴/画线工具/大周期视图 + 背景K线持久化修复，2026-08-30）：
  1. OKX 风格 OHLC 图例：悬停显示对应K线 开/高/低/收/涨跌幅（对比前一根收盘）与 EMA 值，未悬停时显示最新一根，可直接读数设置止损止盈；可在图层开关中隐藏。
  2. 十字线时间轴完整化：十字线底部时间标签统一显示北京时间「周几 YYYY-MM-DD HH:mm」。
  3. 图层细粒度开关：指标面板分组（均线/关键价位线/交易与辅助），PDO/PDH/PDL/PDC/OPEN/PRE-H/PRE-L 七条价位线可逐条开关，另支持均线右轴数值标签、价位线文字标题、OHLC 图例开关；设置持久化 localStorage。
  4. 画线工具（参考 TradingView）：水平线/趋势线/射线/矩形/斐波那契回撤（0~1 回撤位 + 1.618/2.618/3.618/4.236 扩展位，带价位标签与交替色带）、点选删除与一键清空；锚点以「5m 逻辑索引+价格」存储与周期无关，画线按会话持久化（pall.drawings.<session_id>），Esc 取消绘制；已完成画线支持编辑：悬停高亮后整体拖动、拖端点圆圈微调（水平线上下拖），画布仅在悬停画线或绘制工具激活时接管指针、其余情况透传给图表平移缩放；修复覆盖画布在非整数 devicePixelRatio 下按缓冲区固有尺寸放大显示导致的画线成比例偏移（显式 CSS 尺寸）。
  5. 大周期视图：图表右上角 5分/15分/60分/日线 切换，前端对 cursor 内 5m bars 就地聚合（美东 RTH 边界分组，无前视），大周期 EMA20 客户端等价重算，形态标记自动吸附聚合K线，切周期自动重置可视范围；用于大周期支撑阻力与相似形态对照（Brooks "look at higher time frame"）。
  6. 后端 bug 修复：ReplaySessionORM 新增 context_days 列（alembic 0004），get/advance/back 按会话持久化的 context_days 重建视图——修复恢复会话或推进K线后前N日背景K线丢失的问题（此前 create 之外的视图组装全部按 context_days=0 加载）。
  7. 验证：后端全量 106 项测试全绿；Ruff 0 违规；Mypy 0 错误；前端 npm run build 通过；浏览器端到端实测（悬停图例/时间轴/逐条开关/三种画线/删除/周期切换/刷新持久化/推进与刷新后背景保持）通过。

- Batch 31 完成（画线设置面板 + 盈亏比仓位工具 + 4小时/周线周期，2026-08-30）：
  1. 画线设置面板（双击画线打开，参考 OKX 样式）：斐波那契每条水平可启用/编辑比例/删除/新增并实时显示对应价位，支持恢复默认；水平线可精确输入价位；全部画线可换颜色；确认写回并随会话持久化。
  2. 盈亏比仓位工具（± 按钮）：第 1 点定入场、第 2 点定止损，自动识别多空（止损低于入场=做多）；渲染入场线/止损线/红色风险区/各 R 目标线（默认 1R/2R/3R，标签直接标注「xR（盈亏比 x）+价位」）；双击可自定义目标 R 集合；悬停线条或风险区即可整体拖动，端点手柄微调入场/止损。
  3. 周期补齐：新增 4 小时（09:30 开盘对齐分桶：09:30-13:30 / 13:30-16:00）与周线（美东日历周一分组），周期条变为 5分/15分/60分/4小时/日线/周线。
  4. 修复：拖拽结束的 window mouseup 监听原为条件挂载，快速按下-抬起时 mouseup 先于 effect 到达会导致 dragRef 永久卡死（悬停/点击全部失效）；改为常驻监听 + 空值守卫。盈亏比仓位风险区内部也纳入整体拖动命中。
  5. 验证：前端构建通过；浏览器实测周线/4小时聚合与画线跨周期跟随、盈亏比工具渲染（入场/止损/风险区/1R~3R 标签）、设置面板打开（双击命中盈亏比仓位弹出 3 行目标 R 编辑）；确认写入路径与已验证的画线持久化路径一致。
