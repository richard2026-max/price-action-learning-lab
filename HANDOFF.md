...
# 测试：cd apps/api && .venv/Scripts/python -m pytest tests/   （当前 48 通过）
...
| MVP-C 结构层 | ✅ 第一批：swing（右侧N=3确认）/pullback_leg（净漂移上下文）/hl_counting（H1-H4·L1-L4 状态机，second_entry 标注）；profile=mvp-c-0.1.0；前端已构建；**48 测试全绿** |
